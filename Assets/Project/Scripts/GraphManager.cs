...
public class GraphManager : MonoBehaviour {
	...
	//	public static float SineLevel = 0.25f;
	public static float SineLevel = 0.15f;
	...
	private void CreatePoints () {
		...
		for (int x = 0; x < resolution; x++) {
			for (int z = 0; z < resolution; z++) {
				...
				points[i].color = new Color(0f, p.x + p.z, 0f);
				points[i++].size = 0.1f;
			}
		}
	}
	...
	private static float Sine (Vector3 p, float t){
		return 0.50f +
			SineLevel * Mathf.Sin(4 * Mathf.PI * p.x + 4 * t) * Mathf.Sin(2 * Mathf.PI * p.z + t) +
